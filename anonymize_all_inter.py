...
Video_File = 'C037' # 동영상 파일들이 저장된 폴더명 (C037 폴더에 원본 영상들이 존재한다고 가정)
...
for file in file_list:
    ...
    # interpolatin을 사용하여 비식별화가 제대로 안된 프레임에 추가적으로 비식별화를 수행
    ...
    for key, faces in missed.items():
        ...
        adjusted = adjusted.astype(int)  # 가장 인접해있는 정상 케이스의 전,후 frame 2개에서 검출된 얼굴 좌표의 평균 
